...
sys.path.insert(0, r'/content/DVS-Gesture-Chain-repo')
import dvsgc
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, T_train=None, aug=None, trival_aug=None, mixup_fn=None):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #additional permutation for the convenience of model forward    
        image = image.permute(1, 0, 2, 3, 4)  # (B,T,C,H,W) -> (T,B,C,H,W)
...
def load_data(dataset_dir, distributed, T):
    ...
    dataset_train = dvsgc.DVSGestureChain(root=dataset_dir, split='train', frames_number=T, seq_len=4, class_num=4)
    dataset_test = dvsgc.DVSGestureChain(root=dataset_dir, split='test', frames_number=T, seq_len=4, class_num=4)
